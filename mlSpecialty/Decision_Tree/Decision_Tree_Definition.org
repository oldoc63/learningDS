...
Which of the following datasets has the smallest Gini impurity?
    ...
    - A set of 100 objects - 20 label A, 80 label B.

    - *A set of 100 objects - 5 label A, 95 have label B.*
      (This has an impurity of 0.095)

    - A set of 100 objects - 10 label A, 90 have label B.

    - A set of 100 objects - 50 label A, 50 have label B.
      (This has an impurity of 0.5)
...
[[./employee_attrition.png]]
    ...
    - BusinessTravel_Travel_Frequently
      This has the largest information gain so will be the first split in the tree.
...
Which of the following statements about the decision tree implementation in scikit-learn is TRUE?
    ...
    - ~max_depth~, ~criterion~, ~min_impurity_decrease~, and ~min_samples_leaf~ can all be tuned which can result in better tree performance.
      Correct! These are all hyper-parameters in decision trees which can be changed depending on your data and results and can lead to better performing models.

    - The default criterion is ~entropy~.

** Question 6
The following tree was built on the [[https://www.kaggle.com/pavansubhasht/ibm-hr-analytics-attrition-dataset/version/1][IBM attrition data set]], predicting what employees will stay or leave within the year.

What is the information gain when splitting at the node Age<=24?

[[./IBM_attrition_data_set.png]]

    - 0.125
      The information gain is the difference in the initial Gini impurity, 0.5, and the weighted Gini impurity after the split, (16/39*.305+23/39*.423).

** Question 7
The following tree was built on the [[https://www.kaggle.com/pavansubhasht/ibm-hr-analytics-attrition-dataset/version/1][IBM attrition data set]], predicting what employees will stay or leave within the year.

What is the initial Gini impurity of the dataset (stay vs quit) and what split maximizes the information gain on this set?

[[./IBM_attrition_data_set.png]]

     - 0.454, TotalWorkingYears
       The root of the tree is labeled with the initial Gini impurity and the first split is chosen that maximizes the information gain.

** Question 8
The following tree was built on the [[https://www.kaggle.com/pavansubhasht/ibm-hr-analytics-attrition-dataset/version/1][IBM attrition data set]], predicting what employees will stay or leave within the year.

What is the predicted value for an employee with NumCompaniesWorked=5, TotalWorkingYears = 2, DistanceFromHome = 5, and Age=22 and what is the Gini impurity of the final leaf?

    - Quit, 0.305
      Correct! Following the feature splits in the tree and our data values, the result will be split left-left-left, ending up at the leftmost leaf.
...
What does scikit-learn's DecisionTreeClassifier's .fit() method do?
    ...
    - *The .fit() method creates the tree according to the training data and training labels.*
      Correct! This is where the tree gets created based on the training set.
